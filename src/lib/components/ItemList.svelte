...
<div
  class="flex h-full w-full flex-col border-4 border-border bg-card shadow-[6px_6px_0px_0px_var(--color-border)]"
>
  <!-- Header: Inverted Colors via primary/primary-foreground -->
  <div
  ...
  >
    ...
    <span class="font-mono text-xs font-bold opacity-70">
      {items.length} ITEMS
    </span>
  </div>
  ...
  >
    {#if items.length === 0}
      <div
        class="absolute inset-0 z-0 flex flex-col items-center justify-center opacity-50"
      >
        <span class="font-mono text-3xl font-black uppercase text-foreground"
          >EMPTY</span
        >
        <span class="font-mono text-xs font-bold uppercase text-foreground"
